...
summary = commandArgs(trailingOnly=T)[1]
df <- read.csv(file=summary, sep="\t", header = TRUE)
...
library(yaml)
config = commandArgs(trailingOnly=T)[3]
config <- read_yaml(config)
#cconfig <- read_yaml("./config.yml")
design = commandArgs(trailingOnly=T)[5]
design <- paste(readLines(design), collapse=" ")
# design <- paste(readLines("./design.txt"), collapse=" ")
```
...
```{r setup, echo=FALSE}
library(DT)
# condition: if there is a RIN column in Sample_preparations.tsv, prints a statement and the table; if there is no RIN column, or no Sample_preparations.tsv file, print "RIN values are not available for this project."
...
Quality Assessment of the RNA samples can be found [here](./DESeq2/metadata/Sample_preparations.tsv). The RNA Integrity Number (RIN) value has a maximum value of 10.
\
")
...
df = commandArgs(trailingOnly=T)[2]
df <- read.csv(file=df, sep="\t", header = FALSE)
# df <- read.csv(file="./software_versions.csv", sep="\t", header = FALSE)
```
...
Quality control of the data was performed using *FASTQC*. Details on the quality control parameters are available in the `MultiQC` report of all samples available [here](./MultiQC/multiqc_report.html). `FASTQC` reports for individual samples are additionally available [here](`r commandArgs(trailingOnly=T)[6]`).
...
The raw count table can be found [here](./DESeq2/raw_counts/raw_counts.txt). The normalized count table is available [here](./DESeq2/results/count_tables): rlog_transformed.read.counts.tsv.
...
knitr::include_graphics("./DESeq2/results/plots/PCA_plot.png")
```
...
```{r, echo=F, out.width="2000px", dpi=1200, fig.cap="", fig.align='center'}
knitr::include_graphics("./DESeq2/results/plots/Heatmaps_of_distances.png")
```
...
The list of all the differentially expressed genes is shown below; The full table, where the differentially expressed genes are marked with DE in the filter1 column, can be found [here](./DESeq2/results/final/final_list_DESeq2.tsv).
...
Contrastsfile = commandArgs(trailingOnly=T)[4]
