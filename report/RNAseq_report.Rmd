...
output:
  html_document:
    ...
    css: ./input/corp-styles.css
    highlight: pygments
#   code_folding: none
#   theme: spacelab
    pdf_document: true
#   self_contained: no
bibliography: ./input/references.bibtex
---
...
df <- read.csv(file="./input/summary.tsv", sep="\t", header = TRUE)
...
design <- paste(readLines("../DESeq2/input/design.txt"), collapse=" ")
```
...
<img src="./input/logo.png" style="position:absolute;top:0px;right:0px;" height="200" width="200" />
...
QAfile = "./input/QualityAssessment.csv"
no_RIN = FALSE
RIN=FALSE
...
Quality Assessment of the RNA samples can be found [here](./input/QualityAssessment.csv). The RNA Integrity Number (RIN) value has a maximum value of 10.
\
")
...
df <- read.csv(file="./input/pipeline_info/software_versions.csv", sep="\t", header = FALSE)
```
...
Quality control of the data was performed using *FASTQC*. Details on the quality control parameters are available in the `MultiQC` report of all samples available [here](./input/results/MultiQC/multiqc_report.html). `FASTQC` reports for individual samples are additionally available [here](./input/results/fastqc.zip).
...
```{r}
library(DT)
df_DE <- read.table("./input/results/MultiQC/multiqc_data/multiqc_general_stats.txt", header=TRUE, sep="\t")
...
metadata <- df_DE <- read.table("./DESeq2/zips/DESeq2/metadata/Sample_preparations.tsv", header=TRUE, sep="\t")
...
knitr::include_graphics("./input/results/MultiQC/multiqc_plots/svg/mqc_star_alignment_plot_1_pc.svg")
```
\

#### Number of reads {-}
```{r, echo=F, out.width="160%", dpi=1200, fig.cap="STAR: Alignment Scores", fig.align='center'}
knitr::include_graphics("./input/results/MultiQC/multiqc_plots/svg/mqc_star_alignment_plot_1.svg")
```
</center>
\
...
knitr::include_graphics("./input/results/MultiQC/multiqc_plots/svg/mqc_rseqc_read_distribution_plot_1_pc.svg")
```
</center>
...
<center>
```{r, echo=F, out.width="160%", dpi=1200, fig.cap="RSeQC: Read Distribution", fig.align='center'}
knitr::include_graphics("./input/results/MultiQC/multiqc_plots/svg/mqc_rseqc_read_distribution_plot_1.svg")
```
</center>
...
knitr::include_graphics("./input/results/MultiQC/multiqc_plots/svg/mqc_featureCounts_assignment_plot_1_pc.svg")
```
</center>
...
<center>
```{r, echo=F, out.width="160%", dpi=1200, fig.cap="featureCounts: Assignments", fig.align='center'}
knitr::include_graphics("./input/results/MultiQC/multiqc_plots/svg/mqc_featureCounts_assignment_plot_1.svg")
```
</center>
...
The raw count table can be found [here](./DESeq2/zips/DESeq2/raw_counts/raw_counts.txt). The normalized count table is available [here](./DESeq2/zips/DESeq2/results/tables): rlog_transformed.read.counts.tsv.
...
knitr::include_graphics("./DESeq2/zips/DESeq2/results/plots/PCA_plot.png")
```
...
```{r, echo=F, out.width="2000px", dpi=1200, fig.cap="", fig.align='center'}
knitr::include_graphics("./DESeq2/zips/DESeq2/results/plots/Heatmaps_of_distances.png")
```
...
Analysis of the differential gene expression was performed using `DESeq2`. Genes were considered differentially expressed genes (DEG) when the adjusted p-value was lower than 0.05 (padj < 0.05). The adjusted p-value is calculated in the `DESeq2` package with the Benjamini-Hochberg method and helps reduce the number of false postives (not real differentially expressed genes). The model design in DESeq2 was: `r cat(design)`.
The list of all the differentially expressed genes is shown below; The full table, where the differentially expressed genes are marked with DE in the filter1 column, can be found [here](./DESeq2/zips/DESeq2/results/final/final_list_DESeq2.tsv).
...
library(DT)
# condition: if there is a RIN column in QualityAssessment.csv, prints a statement and the table; if there is no RIN column, or no ./QualityAssessment.csv file, print "RIN values are not available for this project."
Contrastsfile = "../DESeq2/input/contrasts.tsv"
contr=FALSE
...
cat("***
Additional tables for all individual contrasts are provided [here](./DESeq2/zips/DESeq2/results/tables):
\
")
...
Plots of the normalized counts were generated for the all the genes. Count normalization was performed by sequencing depth and adding a pseudocount of 1/2 to allow for log scale plotting. The plots are available at [plot_candidate_genes](./DESeq2/zips/DESeq2/results/plots/plots_requested_genes).
